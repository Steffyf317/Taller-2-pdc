...
#### Se desarrolló la función separar_digitos para descomponer un número flotante en los dígitos de su parte entera y decimal. La función primero extrajo la parte entera y, si existía, la parte decimal del número. Luego, separó los dígitos de la parte entera mediante un bucle que los iba obteniendo uno por uno y almacenando en una lista. De manera similar, descompuso la parte decimal en dígitos y los convirtió en enteros.
```python
def separar_digitos(n):
...
#### Se creó la función son_espejos para verificar si dos números enteros son espejos entre sí. La función convirtió ambos números en cadenas y comparó la cadena del primer número con la cadena del segundo número invertida.
```python
def son_espejos(a, b):
...
#### Se desarrolló la función aproximacion_coseno para calcular el coseno de un número a partir de la serie de Taylor. Posteriormente, la función n_terminos se encargó de encontrar cuántos términos de la serie eran necesarios para alcanzar un error menor a los valores especificados en m_list. 
```python
import math
